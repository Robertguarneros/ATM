...
from functions.functions2 import load_departures, load_flights, load_24h, correct_altitude_for_file, dms_to_decimal, trajectories_turn_24L, crosses_fixed_radial, extract_turn
...
if not trajectories:
    st.warning(f"No flights took off from runway 24L during the selected time frame ({selected_time_frame})")
else:
    all_coords = [(float(p['latitude']), float(p['longitude'])) for points in trajectories.values() for p in points]
    ...
    m = Map(location=[(mean_lat+lat_dvor_bcn +lat_punto_costas)/3, (mean_lon+lon_dvor_bcn+lon_punto_costas)/3], zoom_start=13)
    ...
    for flight, points in trajectories.items():
        ...
        trajectory_coords = [(float(p['latitude']), float(p['longitude'])) for p in points]
    ...
    def display_metrics(df_clean, column_name, label):
        ...
        cols[3].metric(f"Mode {column_name}", f"{mode_value:.2f}" if mode_value != "nan" else mode_value)
    ...
    with col2:
        ...
        range_counts = df_clean["Altitude Range"].value_counts(normalize=True).reset_index()
    ...
    m = folium.Map(location=[df_clean["Latitude"].mean(), df_clean["Longitude"].mean()], zoom_start=14)
    ...
    fig = px.scatter(
        ...
        labels={"Altitude": "Altitude (ft)", "Aircraft_ID": "Aircraft ID"},  # Custom axis labels
        hover_data={"Altitude": True, "Aircraft_ID": True}  # Display Aircraft ID and Altitude on hover
    )
