...
if selected_runway == "24L and 06R":
    ...
    if combined_results:
        ...
        flattened_results = [item for sublist in combined_results for item in sublist] if isinstance(combined_results[0], list) else combined_results
    ...
    if selected_runway_results:  # Check if there are results
        # Flatten the data structure
        flattened_results = [item for sublist in selected_runway_results for item in sublist] if isinstance(selected_runway_results[0], list) else selected_runway_results
        df = pd.DataFrame(flattened_results)
    else:
        st.warning(f"No data available for the selected runway ({selected_runway}) in the selected time frame. Try changing time frame or runway.")
        st.stop()
...
if "time" in df.columns:
    try:
        ...
        st.error("The 'time' column contains invalid values. Please check the input data.")
        st.stop()
else:
...
if df.empty:
    st.warning(f"No flights detected for runway {selected_runway} in the selected time frame.")
    st.stop()
