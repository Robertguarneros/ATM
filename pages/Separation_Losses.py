...
df_bars = pd.DataFrame(
    results_loa,
    columns=["Front Flight", "Rear Flight", "Distance (NM)", "Category Front", "Category Rear", 
             "Min Separation", "Same SID", "Group Rear", "Compliant"]
)
...
with col1:
    ...
    aircraft_type_counts = df_bars.groupby(['Category Front']).size().reset_index(name='Number of Aircraft')
    
    # Crear gráfico de barras utilizando 'Category Front'
    bar_chart_aircraft_type = alt.Chart(aircraft_type_counts).mark_bar().encode(
        y=alt.Y("Category Front:N", title="Category Front", sort="-x"),  # Eje Y: 'Category Front'
        x=alt.X("Number of Aircraft:Q", title="Number of Aircraft"),  # Eje X: 'Number of Aircraft'
        color=alt.Color("Category Front:N", scale=alt.Scale(scheme="category10")),  # Colores por 'Category Front'
        tooltip=["Category Front:N", "Number of Aircraft:Q"]  # Información en el tooltip
    ).properties(
...
for (front_category, rear_category, condition), min_distance in loa_separation.items():
    ...
    subset_data = df_bars[
        ...
        (df_bars['Same SID'] == condition)  # Cambia esto por la columna correspondiente
    ]
    ...
    if not subset_data.empty:
        ...
        with col1:  # Mostrar estadísticas de distancia de separación (a la izquierda)
            st.subheader(f"Statistics for Distance Between Flights: {front_category} → {rear_category}")

            # Agrupar por las columnas relevantes (categorías front y rear)
            group_stats = subset_data.groupby(["Category Front", "Category Rear"]).agg(
                mean_distance=("Distance (NM)", "mean"),
                std_distance=("Distance (NM)", "std"),
                median_distance=("Distance (NM)", "median"),
                count=("Distance (NM)", "size")
            ...
            if not group_stats.empty:
                for _, stat_row in group_stats.iterrows():
                    col4, col5 = st.columns(2)  # Dividir las métricas en dos columnas para mejor visualización
                    with col4:
                        st.metric("Mean (Average)", f"{stat_row['mean_distance']:.2f} NM")
                        st.metric("Standard Deviation", f"{stat_row['std_distance']:.2f} NM")
                    with col5:
        ...
        with col3:
            ...
            compliance_counts = subset_data["Compliant"].value_counts(normalize=True) * 100
            compliance_data = pd.DataFrame({
                "Category": ["Compliant" if k else "Non-Compliant" for k in compliance_counts.index],
                "Percentage": compliance_counts.values
            })
            ...
            if not compliance_data.empty:
                pie_chart = alt.Chart(compliance_data).mark_arc().encode(
                    theta=alt.Theta("Percentage:Q", title="Percentage"),
                    color=alt.Color("Category:N", scale=alt.Scale(domain=["Compliant", "Non-Compliant"], range=["steelblue", "orange"])),
                    tooltip=["Category", "Percentage"]
                ).properties(
                    title="Compliance Percentage",
                    width=300,
