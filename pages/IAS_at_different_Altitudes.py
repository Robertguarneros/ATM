...
from functions.functions2 import extract_IAS, load_24h, load_departures, load_flights
...
if df.empty:
    ...
    for altitude in [850, 1500, 3500]:
        ...
        cols = st.columns(
            [1.5, 2, 2, 2, 2]
        )  # Proporciones de las columnas, la columna 0 es más pequeña, las demás son más grandes.
        cols[0].markdown(
    ...
    ias_distribution_total = (
        filtered_df.groupby("IAS_Range").size().reset_index(name="Count")
    )
    ...
    with col1:
        range_counts = (
            filtered_df["IAS_Range"].value_counts(normalize=True).reset_index()
        )
        ...
        st.altair_chart(
            ...
            .properties(title="Distribution of IAS by Range", width=400, height=400),
    ...
    with col2:
        st.altair_chart(
            ...
            .properties(title="IAS Distribution by  Altitude", width=400, height=400),
            use_container_width=True,
        )
    ...
    fig = px.scatter(
        ...
        category_orders={
            "Altitude": [850, 1500, 3500]
        },  # Define the domain for the X-axis
        hover_data={
            "Altitude": False,
            "IAS": True,
            "Aircraft_ID": True,
        },  # Control the hover data display
    )
    ...
    )  # Ensure the plot uses the full width of the container
