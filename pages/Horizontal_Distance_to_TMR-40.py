...
from functions.functions3 import (
    calculate_min_distance_to_TMR_40_24L_global,
    load_24h,
    load_departures,
    load_flights,
)
...
st.write(
    "This dashboard provides insights into the horizontal distance to TMR-40 for departures from runway 24L, including statistics, distributions, and detailed visualizations."
)
...
df_grouped = (
    df.groupby("Time (hours)")
    .agg({"Minimum Distance (NM)": ["mean", "count"]})
    .reset_index()
)
...
with col1:
    st.altair_chart(
        ...
        .properties(title="Distribution of Minimum Distances", width=400, height=400),
...
st.altair_chart(
    ...
    .properties(title="Minimum Distance to TMR-40 by Flight", width=1400, height=600),
    use_container_width=True,
)
...
st.altair_chart(
    ...
    .properties(title="Mean Distance by Hourly Intervals", width=800, height=400),
    use_container_width=True,
)
